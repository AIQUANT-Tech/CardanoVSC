...
export const haskellProvider = vscode.languages.registerCompletionItemProvider(
    { language: 'haskell', scheme: 'file' },
    {
        ...
        // Functions
        const functions = [
            { label: 'map', detail: 'Applies a function to each element of a list.' },
            ...
            { label: 'txSignedBy', detail: 'Checks if a transaction is signed by a given public key.' },
            ...
            { label: 'sequence', detail: 'Transforms a list of actions into an action that produces a list.' },
        ...
        functions.forEach(fn => {
            ...
            completions.push(item);
                ...
                pragmas.forEach(pragma => {
                    const item = new vscode.CompletionItem(pragma, vscode.CompletionItemKind.Snippet);
                ...
                // Add Closing Pragma
                const closingPragma = new vscode.CompletionItem("#-}", vscode.CompletionItemKind.Snippet);
                closingPragma.detail = "Close pragma";
                closingPragma.insertText = "#-}";
                completions.push(closingPragma);
            ...
            const plutusIdentifiers = [
                ...
                { label: 'PlutusTx', detail: 'Contains functions and utilities for Plutus code' },
                { label: 'Contract', detail: 'Defines a Plutus smart contract' },
                { label: 'mkValidator', detail: 'Creates a custom validator script' },
                { label: 'txSignedBy', detail: 'Checks if a transaction is signed by a specific public key' },
                { label: 'BuiltinByteString', detail: 'Represents a built-in byte string' },
                ...
                { label: 'TxOutRef', detail: 'Represents a reference to a transaction output' },
                { label: 'ScriptContext', detail: 'Provides contextual information about the transaction' },
            ...
            plutusIdentifiers.forEach(identifier => {
                const item = new vscode.CompletionItem(identifier.label, vscode.CompletionItemKind.Class);
             ...
             // Type Classes
             const typeClasses = [
                { label: 'Eq', detail: 'Equality type class for types that can be compared for equality' },
                { label: 'Ord', detail: 'Ordering type class for types that can be compared for ordering' },
                { label: 'Show', detail: 'Type class for converting values to strings' },
                { label: 'Functor', detail: 'Type class for types that can be mapped over' },
                { label: 'Monad', detail: 'Type class for types supporting monadic operations' }
            ];

            typeClasses.forEach(typeClass => {
                const item = new vscode.CompletionItem(typeClass.label, vscode.CompletionItemKind.Class);
             ...
             const types = [
                ...
                { label: 'String', detail: 'Represents a sequence of characters (i.e., text)' },
                { label: 'Float', detail: 'Represents floating point numbers' },
                { label: 'Double', detail: 'Represents double precision floating point numbers' },
                { label: 'Maybe', detail: 'Represents a value that may or may not exist' },
                { label: 'List', detail: 'Represents a list of elements' },
                { label: 'Tuple', detail: 'Represents a tuple, a fixed-size collection of elements' },
                { label: 'Either', detail: 'Represents a value that can be one of two types (a disjunction)' },
                { label: 'IO', detail: 'Represents a computation that performs I/O and produces a result' },
                ...
                { label: 'User DefinedType', detail: 'Represents any user-defined type in Haskell' },
                { label: 'Monad', detail: 'Represents a type class for monads' },
                { label: 'Functor', detail: 'Represents a type class for functors' },
                { label: 'Applicative', detail: 'Represents a type class for applicatives' }
            ];
