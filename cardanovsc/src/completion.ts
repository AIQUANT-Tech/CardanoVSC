...
class Trie {
  ...
  private collect(node: TrieNode, prefix = "", results: string[] = []): string[] {
    if (node.isEndOfWord && node.moduleName) {
      results.push(node.moduleName);
    }
...
export const haskellProvider = vscode.languages.registerCompletionItemProvider(
  { language: "haskell", scheme: "file" },
  {
    async provideCompletionItems(
      ...
      const searchQuery = linePrefix.replace(/^import\s*/, "");
      const matchingModules = moduleTrie.search(searchQuery);
      
      matchingModules.forEach((moduleName) => {
