...
export class extensionCommand {
 ...
 private  registerCommands(): void {
    const commands = [
      ...
      { command: 'cardanovsc.get_pool_stats', callback: this.getPoolStats.bind(this) },
      { command: 'cardanovsc.get_pools_list', callback: this.getPoolsList.bind(this) },
      { command: 'cardanovsc.get_pools_expiring', callback: this.getPoolsExpiring.bind(this) },
      { command: 'cardanovsc.get_pools_expired', callback: this.getPoolsExpired.bind(this) },
      { command: 'cardanovsc.get_asset_details', callback: this.getAssetDetails.bind(this) },
      { command: 'cardanovsc.get_assets_by_policyId', callback: this.getAssetsByPolicyId.bind(this) },
      { command: 'cardanovsc.get_assets_by_address', callback: this.getAssetsByAddress.bind(this) },
      { command: 'cardanovsc.get_transaction_details', callback: this.getTransactionDetails.bind(this) },
      { command: 'cardanovsc.get_transaction_list_by_address', callback: this.getTransactionListByAddress.bind(this) },
    { command: 'cardanovsc.get_stake_key_details', callback: this.getStakeKeyDetails.bind(this) },
    { command: 'cardanovsc.get_addresses_associated_with_stake_key', callback: this.getAddressesAssociatedWithStakeKey.bind(this) },
    { command: 'cardanovsc.get_network_details', callback: this.getNetworkDetails.bind(this) },
    { command: 'cardanovsc.get_network_protocol_details', callback: this.getNetworkProtocolDetails.bind(this) },
    { command: 'cardanovsc.get_cc_hot_details', callback: this.getCCHotDetails.bind(this) },
    { command: 'cardanovsc.get_cc_member_details', callback: this.getCCMemberDetails.bind(this) },
    { command: 'cardanovsc.get_committee_information', callback: this.getCommitteeInformation.bind(this) },
    { command: 'cardanovsc.get_committee_members', callback: this.getCommitteeMembers.bind(this) },
    { command: 'cardanovsc.get_drep_information', callback: this.getDRepInformation.bind(this) },
    { command: 'cardanovsc.get_dreps_list', callback: this.getDRepsList.bind(this) },
    { command: 'cardanovsc.get_governance_action', callback: this.getGovernanceAction.bind(this) }
  ...
  private async getPoolStats(): Promise<void> {
    ...
    if (!poolId) {
      vscode.window.showErrorMessage('Pool ID is required to fetch pool stats.');
      return;
    }
  ...
  private async getPoolsList(): Promise<void> {
    ...
    if (!pageNo) {
      vscode.window.showErrorMessage('Page number is required to fetch pools list.');
      return;
    }
  ...
  private async getPoolsExpiring(): Promise<void> {
    ...
    if (!pageNo) {
      vscode.window.showErrorMessage('Page number is required to fetch pools expiring list.');
      return;
    }
  ...
  private async getPoolsExpired(): Promise<void> {
    ...
    if (!pageNo) {
      vscode.window.showErrorMessage('Page number is required to fetch expired pools list.');
      return;
    }
  ...
  private async getAssetDetails(): Promise<void> {
    ...
    if (assetId && assetId.length >= 56) {
      ...
      if (fingerprint && fingerprint.length <= 200) {
        ...
        vscode.window.showErrorMessage('Please enter either a valid assetId (min length 56) or a valid fingerprint (max length 200).');
      }
    }
  }
  private async getAssetsByPolicyId(): Promise<void> {
    ...
    const policyId = await vscode.window.showInputBox({
      ...
      placeHolder: 'e.g., policy123456789012345678901234567890123456789012345678901234',
    });

    if (!policyId || policyId.length !== 56) {
      vscode.window.showErrorMessage('The policyId must be 56 characters long.');
      return;
    }
  ...
  private async getAddressesAssociatedWithStakeKey(): Promise<void> {
    ...
    // Prompt user to input the reward address
    const rewardAddress = await vscode.window.showInputBox({
      prompt: 'Enter the stake key reward address (maximum length is 58 characters)',
      placeHolder: 'e.g., stake1u9xyz...',
      validateInput: (input) => {
        if (input.length > 58) {
  ...
  private async getCCHotDetails(): Promise<void> {
    ...
    const hotHex = await vscode.window.showInputBox({
      ...
      validateInput: (input) =>
        input.length === 58 ? null : 'hotHex must be exactly 58 characters long.',
    });
  ...
  private async getCCMemberDetails(): Promise<void> {
    ...
    const coldHex = await vscode.window.showInputBox({
      ...
      validateInput: (input) =>
        input.length === 58 ? null : 'coldHex must be exactly 58 characters long.',
    });
  ...
  private async getDRepInformation(): Promise<void> {
    ...
    const dRepIdInput = await vscode.window.showInputBox({
      ...
      validateInput: (input) =>
        input.length !== 58
          ? 'DRep ID must be exactly 58 characters.'
          : null,
    });
  ...
  private async getDRepsList(): Promise<void> {
    ...
    const searchTermInput = await vscode.window.showInputBox({
      ...
      validateInput: (input) =>
        input.length > 200 ? 'Search term must be less than 200 characters.' : null,
    });
  ...
  private async getGovernanceAction(): Promise<void> {
    ...
    const actionIdInput = await vscode.window.showInputBox({
      ...
      validateInput: (input) =>
        input.length !== 66
          ? 'ActionId must be exactly 66 characters.'
          : null,
    });
